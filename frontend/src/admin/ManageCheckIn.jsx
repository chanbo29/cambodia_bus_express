...
// { label, urgency } countdown — same logic as ManageBookings.jsx
    ...
    <div className="admin-page">
      ...
      <main className="admin-main">
        <header className="admin-header">
          <div>
            <h1>
              <ClipboardCheck size={22} style={{ verticalAlign: "-4px", marginRight: "8px" }} />
              Manage Check-In
            </h1>
        ...
        <section className="admin-stats">
          <div>
            ...
            <h3>{loading ? "—" : filtered.length}</h3>
          </div>
          <div>
            <CheckCircle2 />
            <span>Checked In</span>
            <h3>{loading ? "—" : checkedInCount}</h3>
          </div>
          <div>
            <Circle />
            <span>Not Checked In</span>
            <h3>{loading ? "—" : filtered.length - checkedInCount}</h3>
          </div>
        </section>
        ...
        <section className="admin-card">
          ...
          <div className="ci-filters-row">
            ...
            <div className="ci-date-filter">
              ...
              />
              {dateFilter && (
                <button onClick={() => setDateFilter("")} title="Show all dates">
                  Clear
                </button>
              )}
            </div>
                ...
                {filtered.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className={b.checked_in ? "ci-row-done" : ""}>
                      ...
                      <td>
                        {b.passenger_name}
                        <span className="ci-pax-count">
                          {passengerCount(b)} pax
                        </span>
                      </td>
                      <td>
                      ...
                      <td>
                        <span className={`mb-checkin-badge ${urgency}`}>{label}</span>
                      </td>
                      <td>
                        <button
                          className={`ci-toggle-btn ${b.checked_in ? "done" : ""}`}
                          onClick={() => toggleCheckedIn(b)}
                        >
                          {b.checked_in ? (
                            <>
                ...
                {!loading && filtered.length === 0 && (
                  <tr>
                    <td colSpan="7" className="empty-table">
                      No upcoming departures for this date
                    </td>
                  </tr>
                )}
              </tbody>
