...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    API.get("/users/").then((res) => {
      ...
      setNewUsersCount(users.filter((u) => new Date(u.date_joined).getFullYear() === thisYear).length);
  ...
  const monthlyData = MONTH_NAMES.map((name, index) => {
    const mb = bookings.filter((b) => { if (!b.travel_date) return false; const d = new Date(b.travel_date); return d.getMonth() === index && d.getFullYear() === today.getFullYear(); });
  ...
  const routeMap = {};
  bookings.forEach((b) => { const key = `${b.from_city} → ${b.to_city}`; if (!routeMap[key]) routeMap[key] = { route:key,bookings:0,revenue:0 }; routeMap[key].bookings+=1; routeMap[key].revenue+=Number(b.total_price||0); });
  ...
  const vehicleMap = {};
  bookings.forEach((b) => { const key = b.vehicle_type||"Unknown"; if (!vehicleMap[key]) vehicleMap[key]={type:key,bookings:0,revenue:0}; vehicleMap[key].bookings+=1; vehicleMap[key].revenue+=Number(b.total_price||0); });
  ...
  bookings.forEach((b) => { if (!b.travel_date) return; const [y,m,d] = b.travel_date.split("-").map(Number); dowCounts[new Date(y,m-1,d).getDay()]+=1; });
    ...
    <div className="admin-page">
      ...
      <main className="admin-main">
        ...
        <section className="admin-card">
          <div className="card-title"><h2>Monthly Revenue</h2><p>{loading ? "Loading..." : `${monthlyData.length} month(s) with activity`}</p></div>
          {monthlyData.length === 0 && !loading && <p className="reports-empty">No booking data yet to report on.</p>}
          {monthlyData.length > 0 && (
            <>
              <div className="reports-chart">
                {monthlyData.map((m) => (
                  <div className="reports-bar-col" key={m.month}>
                    <div className="reports-bar-track"><div className="reports-bar-fill" style={{height:`${(m.revenue/maxRevenue)*100}%`}} title={`$${m.revenue.toFixed(2)}`} /></div>
              ...
              <div className="route-table">
                <table>
                  ...
                  <tbody>{monthlyData.map((m) => <tr key={m.month}><td>{m.month}</td><td>{m.bookings}</td><td>${m.revenue.toFixed(2)}</td></tr>)}</tbody>
                </table>
              </div>
            </>
          )}
        ...
        <section className="admin-card">
          <div className="card-title"><h2>Bookings by Day of Week</h2><p>Reveals weekly demand patterns across all routes</p></div>
